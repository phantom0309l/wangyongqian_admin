    ...
    <div class="schedule-addtpl">
        <el-form :model="scheduletpl" :rules="rules" ref="scheduletpl" label-width="100px" class="form-scheduletpl">
            ...
            <el-form-item label="状态">
                <el-radio-group v-model="scheduletpl.status">
            ...
            <el-form-item>
                <el-button type="primary" @click="submitForm('scheduletpl')">保存</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            findXProvince_index(xprovinceid) {
                let self = this;
                self.xprovinces.forEach((value, index) => {
                    if (value.id === xprovinceid) {
                        self.xprovince_index = index;
                        return false;
                    }
                });
                return self.xprovince_index;
            },
            findXCity_index(xcityid) {
                let self = this;
                self.xprovinces[self.xprovince_index].children.forEach((value, index) => {
                    if (value.id === xcityid) {
                        self.xcity_index = index;
                        return false;
                    }
                });
                return self.xcity_index;
            },
            handleItemChange(item) {
                ...
                if (item.length === 1) {
                    self.findXProvince_index(item[0]);
                    // self.xprovinces.forEach((value, index) => {
                    //     if (value.id === item[0]) {
                    //         self.xprovince_index = index;
                    //         return false;
            ...
            fetchData() {
                let self = this;
                let url = api.get('scheduletplmgr.modify');

                common.ajax({
                    url: url,
                    params: {
                        scheduletplid: self.scheduletpl.id
                    },
                    isNeedLoading: false,
                    done: function (data) {
                        ...
                        self.scheduletpl = data.scheduletpl;

                        if (self.scheduletpl.address.length > 0) {
                            let xprovinceid = self.scheduletpl.address[0];
                            self.findXProvince_index(xprovinceid)
                            common.fetchXCitys(xprovinceid, (xcitys) => {
                                self.xprovinces[self.xprovince_index].children = xcitys;

                                if (self.scheduletpl.address.length > 1) {
                                    let xcityid = self.scheduletpl.address[1];
                                    common.fetchXCountys(xcityid, (xcountys) => {
                                        self.xprovinces[self.xprovince_index].children[self.xcity_index].children = xcountys;
                                    })
                                }
                            });
                        }
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        let self = this;
                        let url = api.get('scheduletplmgr.modifypost');
        ...
        created: function () {
            this.scheduletpl.id = this.$route.params.scheduletplid;
